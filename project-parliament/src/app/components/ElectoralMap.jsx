...
import mapDataNew from '../../../public/650map_new.json';
...
const ElectoralMap = ({onMapClick}) => {
  ...
  useEffect(() => {
    ...
    const svg = d3.select(svgRef.current)
      .attr('height', 820) // Replace with your map.height
      .attr('width', 800) // Replace with your map.width
      .attr('preserveAspectRatio', 'xMinYMin meet')
      .attr('viewBox', '00 0 1000 1000'); // Replace with your map.width and map.height
    ...
    const geoData = topojson.feature(mapDataNew, mapDataNew.objects.map).features;
  ...
  const handlePartyChange = (event) => {
    setSelectedPartyType(event.target.value);
  };

  const handleAgeChange = (event) => {
    setSelectedAgeType(event.target.value);
  };
  const handleEthChange = (event) => {
    setSelectedEthType(event.target.value);
  };
  ...
  return (
    <div>
      <div>
        <label htmlFor="dropdown">Choose a map type:</label>
        <select id="dropdown" value={selectedMapType} onChange={handleChange}>
      ...
      <div>
        <label htmlFor="dropdown">Choropleth by vote share:</label>
        <select id="dropdown" value={selectedPartyType} onChange={handlePartyChange}>
      ...
      <div>
        <label htmlFor="dropdown">Choropleth by age:</label>
        <select id="dropdown" value={selectedAgeType} onChange={handleAgeChange}>
      ...
      <div>
        <label htmlFor="dropdown">Choropleth by ethnicity:</label>
        <select id="dropdown" value={selectedEthType} onChange={handleEthChange}>
      ...
      <svg className="border-r-4 border-b-4 border-r-gray-950  border-b-gray-950" ref={svgRef}></svg>
    </div>
  );
}
